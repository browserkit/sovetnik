...
function scopify(scope, options) {
    ...
    return function(root) {
        ...
        root.walkRules(function(rule) {
            if (!rule.selectors || (rule.parent.type === 'atrule' && rule.parent.name !== 'media')) {
                return rule;
            }

            rule.selectors = rule.selectors.map(function(selector) {
                if (isScopeApplied(selector, scope) || containsScope(selector, scope)) {
                    return selector;
                }
...
/**
 * Determine if selector contains scope
 *
 * @param {string} selector
 * @param {string} scope
 */
function containsScope(selector, scope) {
    return selector.indexOf(scope) !== -1;
}
